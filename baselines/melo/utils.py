...
def tokenize_qa(batch, tokenizer, device, **kwargs):
    ...
    input_sequences = [text + labels for text, labels in zip(batch["text"], batch["labels"])]
    output_sequences = copy.deepcopy(input_sequences)
    ...
    loss_mask = attention_mask - q_mask
