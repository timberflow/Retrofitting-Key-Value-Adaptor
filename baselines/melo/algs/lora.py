...
from baselines.melo.hooks import lora_backward_hook
...
class LORA(torch.nn.Module):
    ...
    def edit(self, tokens):
        ...
        for i in range(self.config.grace.num_iter):
            ...
            # loss = -torch.nn.functional.log_softmax(probs, dim = -1)[tokens["loss_mask"] == 1].mean()
            # loss = outputs.loss
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
    ...
    def forward(self, *args, **kwargs):
        return self.model.model.forward(*args, **kwargs)
